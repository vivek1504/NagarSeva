...
const randomAround = (v: number, meters = 20) =>
  v + (Math.random() * 2 - 1) * (meters / 111000);
...
async function processImage(
  ...
  try {
    ...
    await fs.promises.writeFile(
      imagePath,
      Buffer.from(piexif.insert(exif, jpeg), "binary"),
    );
...
surveyorRouter.post(
  ...
  async (req, res) => {
    ...
    if (active) return res.status(400).json({ success: false });
